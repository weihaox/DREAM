...
image_path = 'data/processed_depth_data/subj{:02d}/nsd_train_stim_sub{}.npy'.format(sub,sub)
train_images = batch_generator_external_images(data_path = image_path)

image_path = 'data/processed_depth_data/subj{:02d}/nsd_test_stim_sub{}.npy'.format(sub,sub)
...
#   print(x.shape) # depth controlnet_aux is (30, 64, 64, 3), depth from midas is (30, 64, 64), but vdvae takes (30, 64, 64, 3)
  x = np.repeat(x[:, :, :, np.newaxis], 3, axis=3)
#   print(x.shape, np.array_equal(x[:,:,:,0],x[:,:,:,1])) 
...
np.savez("data/extracted_depth_features/subj{:02d}/nsd_vdvae_features_31l.npz".format(sub),train_latents=train_latents,test_latents=test_latents)
